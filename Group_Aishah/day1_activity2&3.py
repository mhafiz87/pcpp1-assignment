class Machine:
    def __init__(
        self, machine_id, equipment_type, product_catergory="", product_type=""
    ):
...
class SpeakersAssemblyMachine(Machine):
    def add_run(self, run_start, run_end, units_produced, product):
        if isinstance(product, Speakers):
            print("Speaker will be add in SpeakersAssemblyMachine")
        else:
            raise ValueError("Your device is not speakers")
...
machine_5673 = Machine(
    "MC-5673", "Speaker Assembly Machine", "Audio Devices", "Speakers"
)
...
sam = SpeakersAssemblyMachine(
    "MC-5673", "Speaker Assembly Machine", "Audio Devices", "Speakers"
)
sam.add_run(
    product=Camera("visual", "camera", "black", "LG"),
    run_start="2024-01-20",
    run_end="2024-01-24",
    units_produced=420,
)
...
try:
    sam.add_run(
        product=Speakers("visual", "camera", "black", "LG"),
        run_start="2024-01-20",
        run_end="2024-01-24",
        units_produced=420,
    )
